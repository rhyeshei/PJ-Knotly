...
- 作成セットアップとビルダーを分けた 2 段階ページ作成
- テンプレート選択またはまっさらな状態からの作成
- ページタイプ別の初期ブロック自動生成
- ブロック単位での本文編集
- 左ペインのブロックパレットからのクリック追加
- ブロックの非表示化による削除
- ブロックの上へ / 下へ並び替え
...
- 独自ダッシュボード上の作成セットアップ
- テンプレート選択またはまっさらな状態からの作成
- 左ペインのブロックパレットからページ構成を組み立てるビルダー
- ページ基本情報の編集
- ブロック本文の編集
- 任意ブロックの追加
- 任意ブロックの非表示化
...
1. `/dashboard/pages/new/` で作成方式を選ぶ
2. タイトル、カテゴリ、対象者、タグ、担当部署、緊急度を設定する
3. 下書きページを作成する
4. `/dashboard/pages/<id>/edit/` の 3 ペインビルダーに遷移する
5. 左ペインのブロックパレットから必要なブロックをクリック追加する
6. 中央で本文を編集し、右ペインでメタ情報を確認・編集する
...
- ページ作成は「作成セットアップ → ブロック選択型ビルダー」の 2 段階です。
- `blank` テンプレートを選ぶと、`page_type=custom` の自由作成ページとして開始できます。
...
- **ページ作成は setup 画面と block builder を分けて考える**
- **左ペインのブロックパレットからクリックで構成を組み立てる**
- **ドラッグ＆ドロップは将来拡張とし、MVPではクリック追加を優先する**
